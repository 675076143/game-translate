...
impl WayshotConnection {
    ...
    pub fn capture_output_frame_get_state(
        ...
        // Realtime capture needs explicit per-frame destruction. The ext-image-copy
        // output path loses its source and session handles, so use wlr-screencopy.
            ...
            if let Some(frame_state) = state.state {
                match frame_state {
                    FrameState::Failed | FrameState::FailedWithReason(_) => {
                        tracing::error!("Frame copy failed");
                        // The compositor retains the shared buffer until the frame is destroyed.
                        frame.destroy();
                        event_queue.roundtrip(&mut state)?;
                        return Err(Error::FramecopyFailed);
                    }
                    FrameState::Finished => {
                        // Flush destruction before the next high-frequency capture.
                        frame.destroy();
                        event_queue.roundtrip(&mut state)?;
                        tracing::trace!("Frame copy finished");
    ...
    fn screenshot_region_capturer(
        ...
        let ext_top_level_support = false;
